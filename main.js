...
const extractBulletPoints = (section) => {
    ...
    section.forEach((item) => {
        ...
        prevY = item.transform[5]; // Update the vertical position (Y-coordinate)
    });
...
const getCleanedItems = async (page) => {
    ...
    textContent.items.forEach((item) => {
        ...
        } else if (previousItem && Math.abs(previousItem.transform[5] - item.transform[5]) < 5) {
...
async function getSectionsFromCleanedItems(CLEANED_ITEMS, page) {
    ...
    const SECONDARY_SECTION_TITLES = ["job", "course", "extracurricular", "objective", "summary", "award", "honor"];
    ...
    CLEANED_ITEMS.forEach((line) => {
        if (
            line.length == 1 &&
            page.commonObjs.get(line[0].fontName).name.toLowerCase().includes("bold") &&
...
async function getWorkExperience(SECTIONS, page) {
    ...
    const hasJobTitle = (item) => JOB_KEYWORDS.some((jobTitle) => item.str.split(/\s/).some((word) => word === jobTitle));
    ...
    const hasMonth = (item) => MONTHS.some((month) => item.str.includes(month) || item.str.includes(month.slice(0, 4)));
    const SEASONS = ["Summer", "Fall", "Spring", "Winter"];
    const hasSeason = (item) => SEASONS.some((season) => item.str.toLowerCase().includes(season.toLowerCase()));
    const hasPresent = (item) => item.str.toLowerCase().includes("present");
    const hasText = (str) => (item) => item.str.toLowerCase().includes(str.toLowerCase());
    ...
    subsections.forEach((subsection) => {
        ...
        output.push({ job_title: job[0].str, date: date[0].str, company: company[0].str, description: bullets });
    });
...
async function getProjects(SECTIONS, page) {
    ...
    subsections.forEach((subsection) => {
        ...
        output.push({ title: title[0].str, date: date[0].str, description: bullets });
    });
...
const getStandardDeviation = (gaps, mean) => {
    const variance = gaps.reduce((sum, gap) => sum + Math.pow(gap - mean, 2), 0) / (gaps.length - 1);
...
const calculateAvgAndStd = (gaps) => {
    return { avg: getAvg(gaps), stdDev: getStandardDeviation(gaps, getAvg(gaps)) };
};
...
function getSubSections(section, page) {
    ...
    section.flat().forEach((item) => {
        ...
        const verticalGap = Math.round(Math.abs(item.transform[5] - previousItem.transform[5]));
    ...
    if (sections.length == 1) {
        ...
        const isLineNewSubsectionByBold = (line, prevLine) => {
            if (
                !page.commonObjs.get(prevLine[0].fontName).name.toLowerCase().includes("bold") &&
                page.commonObjs.get(line[0].fontName).name.toLowerCase().includes("bold") &&
...
async function main() {
    ...
    const CLEANED_ITEMS = await getCleanedItems(page).catch((err) => console.log("Error extracting from pdf", err));

    const FOUND_SECTIONS = await getSectionsFromCleanedItems(CLEANED_ITEMS, page).catch((err) =>
